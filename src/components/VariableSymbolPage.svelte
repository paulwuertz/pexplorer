<script>
	...
	const { var_childs, symbol_version, SymPathByAddr, VariableTypes } = $props();
    ...
    let typeStruct = $derived(VariableTypes.find((val, i, arr) => { return val.name == type}))
		...
		<tr>
			...
			<th>Value</th>
				...
				{#each typeStruct.members as m (m.name)}
                        ...
                        <tr>
                            <td>{m.byte_offset}</td>
                            <td>{#if m.is_pointer}*{/if}{m.name}</td>
                            <td>{m.size}</td>
