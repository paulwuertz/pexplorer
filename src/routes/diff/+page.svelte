<script>
	...
	const updateSelectedSymbols = () => {
		...
		for (const symPath of deletedSymbols) {
			...
			selected_symbols_to_compare[symPath].remark +=
				'Deleted in ' + symbols.selected_versions_to_compare;
			...
			if (selected_symbols_to_compare[symPath].stack_size) {
				selected_symbols_to_compare[symPath].d_stack =
					-selected_symbols_to_compare[symPath].stack_size;
			}
		...
		for (const symPath of Object.keys(selected_symbols)) {
			...
			if (newSymbols.includes(symPath)) {
				...
				if (selected_symbols[symPath].size != selected_symbols_to_compare[symPath].size) {
					selected_symbols[symPath].d_size =
						selected_symbols[symPath].size - selected_symbols_to_compare[symPath].size;
					shouldAdd = true;
				}
				if (
					selected_symbols[symPath].stack_size != selected_symbols_to_compare[symPath].stack_size
				) {
					selected_symbols[symPath].d_stack =
						selected_symbols[symPath].stack_size - selected_symbols_to_compare[symPath].stack_size;
					shouldAdd = true;
				}
			}
	...
	<Container fluid>
		...
		{:else}
			...
			<ul>
				...
				<li>
					...stack size is {Object.values(symbols_to_show)
						.filter((e) => {
							return e['d_stack'];
						})
						.reduce((acc, b) => acc + b['d_stack'], 0)} bytes
				</li>
			</ul>
