...
export async function load({ route, params }) {
	...
	let version = params && Object.hasOwn(params, 'sections') && params.version;
	console.log('sections versel', symbols, params, version, versions.length);
	if (versions.length == 1 && !version) {
		let selVers = versions[0];
		symbols.elfDataProvided = true;
		symbols.selected_version = selVers;
		goto(base + '/#/sections/' + selVers);
		return params;
	} else if (!symbols.elfDataProvided) {
		goto(base + '/#/');
		return params;
	}
	console.log('sections versel', route, params);
	return params;
}
