...
func AddCallGraph(s *SElfReport) {
	...
	for i := 0; i < len(s.Functions); i++ {
		...
		if err == nil || err == gapstone.ErrOK {
			fmt.Println("Disasm:", f.Name, f.Address, f.Address+f.FlashSize)
			for _, insn := range insns {
				if isFnCallInstr(insn.Mnemonic) {
					...
					if err == nil && n == 1 {
						f.Callees = append(f.Callees, FunctionCall{f.Address, uint64(calladdr), false})
					} else {
						f.Callees = append(f.Callees, FunctionCall{CallFrom: f.Address, DynamicCall: true})
						continue
					}
					fnCalled, fnFound := s.Addr2FnMap[uint64(calladdr)]
					if fnFound {
						fnCalled.Callers = append(fnCalled.Callers, FunctionCall{f.Address, uint64(calladdr), false})
					} else {
						//
						fmt.Println("\tcallto:", insn.Address, "->", insn.Mnemonic, insn.OpStr)
					}
